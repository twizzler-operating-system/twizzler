...
fn main() -> Result<()> {
    println!("=== Virtual Table Test Program ===");
...
fn test_virtual_table() -> Result<()> {
    ...
    // Create a virtual table with some columns
    conn.execute(
        "CREATE VIRTUAL TABLE test_data USING twz_vtab('name:TEXT', 'age:INTEGER', 'score:REAL')",
        [],
    )?;
    ...
    println!("\n--- Testing INSERT ---");
    conn.execute(
        "INSERT INTO test_data (name, age, score) VALUES ('Alice', 25, 95.5)",
        [],
    )?;
    
    conn.execute(
        "INSERT INTO test_data (name, age, score) VALUES ('Bob', 30, 87.2)",
        [],
    )?;
    
    conn.execute(
        "INSERT INTO test_data (name, age, score) VALUES ('Charlie', 28, 92.8)",
        [],
    )?;
    
    println!("✓ Inserted 3 records");
    
    // Test SELECT operations (no ID column)
    println!("\n--- Testing SELECT ---");
    let mut stmt = conn.prepare("SELECT name, age, score FROM test_data")?;
    ...
    println!("Records in virtual table:");
    for row in rows {
        let (name, age, score) = row?;
        println!("  Name: '{}', Age: {}, Score: {:.1}", name, age, score);
    }

    println!("\n--- Testing constrained SELECT ---");
    let mut stmt = conn.prepare("SELECT name, age, score FROM test_data WHERE age > 26")?;
    ...
    // Test COUNT query
    println!("\n--- Testing COUNT ---");
    let mut stmt = conn.prepare("SELECT COUNT(*) FROM test_data")?;
