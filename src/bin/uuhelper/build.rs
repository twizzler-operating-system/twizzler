    ...
    for (key, val) in env::vars() {
        if val == "1" && key.starts_with(ENV_FEATURE_PREFIX) {
            ...
            match krate.as_ref() {
                ...
                "test" => continue,  /* over-ridden with 'uu_test' to avoid collision with rust */
                // core crate 'test'
    ...
    for krate in &crates {
        ...
        match krate.as_ref() {
            ...
            "hashsum" => {
                ...
                phf_map.entry("sha3-224sum", map_value_bits.clone());
                phf_map.entry("sha3-256sum", map_value_bits.clone());
                phf_map.entry("sha3-384sum", map_value_bits.clone());
                phf_map.entry("sha3-512sum", map_value_bits.clone());
