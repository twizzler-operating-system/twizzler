...
register_structs! {
    ...
    DistributorRegisters {
        ...
        (0xBFC => _reserved4),
        /// Interrupt Configuration Registers: 2 bits per interrupt, bit 1 set for edge-triggered
...
impl GICD {
    ...
    pub(super) const SPI_ID_RANGE: RangeInclusive<u32> = RangeInclusive::new(32, 1019);
    ...
    /// Configure an SPI as edge- or level-triggered (GICD_ICFGR).
    pub fn set_edge_triggered(&self, int_id: u32, edge: bool) {
        if !Self::SPI_ID_RANGE.contains(&int_id) {
            return;
        }
        let reg = (int_id / 16) as usize;
        let bit = (int_id % 16) * 2 + 1;
        let mut v = self.registers.ICFGR[reg].get();
        if edge {
            v |= 1 << bit;
        } else {
            v &= !(1 << bit);
        }
        self.registers.ICFGR[reg].set(v);
    }
    ...
    pub fn set_interrupt_target(&self, int_id: u32, core: u32) {
        ...
        if int_id < *Self::SPI_ID_RANGE.start() {
            return;
        }

        // Following the algorithm on page 4-107:
        // 1. ITARGETSR num = int_id / 4
        // minus 8 since the banked registers are split off above
        let num = (int_id / 4) as usize - 8;
