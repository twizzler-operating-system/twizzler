...
cfg_if! {
    if #[cfg(any(target_os = "linux", target_os = "android"))] {
        #[path = "linux.rs"]
        mod imp;
    // MUST precede the `unix` arm: twizzler sets `target_family = "unix"`, so a twizzler arm
    // placed after it is unreachable. That is exactly how the previous fork's parker became
    // 110 lines of dead code and the pthread mutex+condvar parker ran on the monitor instead --
    // which costs a TLS lookup and a lazily constructed parker on every contended acquisition,
    // because `unix.rs` sets IS_CHEAP_TO_CONSTRUCT = false and `word_lock.rs` branches on it.
