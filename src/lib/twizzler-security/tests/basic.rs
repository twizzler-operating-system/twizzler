...
fn creation_and_verification() {
    ...
    //     // basically this priv_key needs to be 32 bytes long, if we want the keys to be more
    // adaptable,     // we would need a key struct and abstract it away, since right now the
    // implementation only     // works if we use a hard-coded size
