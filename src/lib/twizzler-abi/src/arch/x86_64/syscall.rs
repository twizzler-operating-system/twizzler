...
pub unsafe fn raw_syscall(call: Syscall, args: &[u64]) -> (u64, u64) {
    ...
    asm!("syscall", inout("rax") num, in("rdi") a0, in("rsi") a1, inout("rdx") a2, in("r10") a3, in("r9") a4, in("r8") a5, lateout("rcx") _, lateout("r11") _, clobber_abi("system"));
    (num, a2)
}
