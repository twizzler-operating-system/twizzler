...
- **gadget** and **test-tiny-http** both depend on the vendored `tiny_http` crate at `src/bin/test-tiny-http/src/tiny-http-twizzler` (path dependency), which is a fork of the third-party `tiny_http` HTTP server library ported to Twizzler's socket/IO primitives.
